...
class api_helpers_Search
{
    ...
    {        
        ...
        if (empty($version)) $version = $default_database;
                
        try
        {
            $this->_db = Zend_Db::factory($databases[$version]['adapter'], $databases[$version]);
        }
        catch (Exception $e)
        {
            ...
            if ((!empty($genus)) && !(empty($species) && !empty($infraspecies)))
            {
                ...
                $sql = " SELECT ssac.id, ssac.kingdom, ssac.phylum, ssac.class, ssac.`order`, ssac.superfamily, ssac.family, ssac.genus, 
                         ...
                         FROM _search_scientific ss
                            LEFT JOIN _search_scientific ssac ON (ss.accepted_species_id = ssac.id OR (ss.status IN (1,4) AND ss.id = ssac.id))
                ...
                foreach ($rows_accepted as $row_ac)
                {
                    ...
                    while ($row_sy = $rows_synonym->fetch())
                    {
                        ...
                        $node_infraspecific_marker = $dom->createElement("infraspecific_marker");
                        $node_infraspecific_marker->appendChild($this->_createFormatedText($dom, $row_sy["infraspecific_marker"]));
                        $node_infraspecies = $dom->createElement("infraspecies");
            ...
            if ((!empty($genus)) && !(empty($species) && !empty($infraspecies)))
            {
                ...
                while ($row_sc = $rows_sciname->fetch())
                {
                    ...
                    if (!empty($row_sc["id"]))
                    {
                        ...
                        $node_infraspecific_marker = $dom->createElement("infraspecific_marker");
                        $node_infraspecific_marker->appendChild($this->_createFormatedText($dom, $row_sc["infraspecific_marker"]));
                        $node_infraspecies = $dom->createElement("infraspecies");
